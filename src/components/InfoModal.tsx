...
const stages = [
  ...
  ['階段三：迭代式分段翻譯', '以約 1,800 Token 的 Markdown 感知區塊翻譯，並帶入術語、角色、前情提要及上一段上下文。'],
  ['階段四：自我校對與動態更新', '逐段檢查漏譯、幻覺、格式與術語，同步更新後續段落使用的上下文。'],
] as const;

export default function InfoModal({ onClose }: { onClose: () => void }) {
  return (
    <AccessibleDialog labelledBy="info-dialog-title" onClose={onClose} className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-slate-900/50">
          <div className="flex items-center gap-3"><div className="bg-blue-600/20 p-2 rounded-xl border border-blue-500/30"><Info className="w-5 h-5 text-blue-400" /></div><h2 id="info-dialog-title" className="text-xl font-semibold text-slate-100">系統說明與翻譯流程</h2></div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-8 text-slate-300">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              {[['區塊大小','約 1,800 Token'],['提取溫度','0.1'],['翻譯溫度','0.2'],['校對溫度','0.0']].map(([label,value]) => <div key={label} className="bg-slate-800/30 p-3 rounded-lg border border-slate-700/30"><span className="text-slate-400 block mb-1">{label}</span><strong className="text-slate-200">{value}</strong></div>)}
            </div>
          </section>
        </div>
    </AccessibleDialog>
