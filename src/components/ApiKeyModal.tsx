...
export default function ApiKeyModal({ googleKey, openaiKey, rememberOnDevice, setGoogleKey, setOpenaiKey, setRememberOnDevice, onClose, onSave }: Props) {
  return (
    <AccessibleDialog labelledBy="api-key-dialog-title" onClose={onClose} overlayClassName="z-[70]" className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative">
        ...
        <div className="p-8 text-center">
          ...
          <h2 id="api-key-dialog-title" className="text-2xl font-semibold mb-2 text-slate-100">API Key 設定</h2>
          <p className="text-slate-400 mb-6 text-sm leading-relaxed">預設只保留到此分頁工作階段結束，並直接傳送給所選的 AI 服務。</p>
          <div className="text-left space-y-4">
            <div><div className="text-sm text-slate-300 mb-2 font-medium flex justify-between"><label htmlFor="google-api-key">Google Gemini API Key：</label><a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-xs text-blue-400 underline">獲取金鑰</a></div><input id="google-api-key" type="password" autoComplete="off" placeholder="AIzaSy..." value={googleKey} onChange={(event) => setGoogleKey(event.target.value)} className="w-full px-4 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-slate-200 focus:ring-2 focus:ring-blue-500 outline-none" /></div>
            <div><div className="text-sm text-slate-300 mb-2 font-medium flex justify-between"><label htmlFor="openai-api-key">OpenAI API Key（選填）：</label><a href="https://platform.openai.com/api-keys" target="_blank" rel="noreferrer" className="text-xs text-emerald-400 underline">獲取金鑰</a></div><input id="openai-api-key" type="password" autoComplete="off" placeholder="sk-proj-..." value={openaiKey} onChange={(event) => setOpenaiKey(event.target.value)} className="w-full px-4 py-2 bg-slate-950 border border-slate-700 rounded-lg text-sm text-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none" /></div>
