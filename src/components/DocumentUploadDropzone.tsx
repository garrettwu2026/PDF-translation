...
export default function DocumentUploadDropzone({ file, totalPages, onFile, disabled = false }: Props) {
  ...
  return (
    ...
    >
      <input ref={inputRef} disabled={disabled} type="file" data-testid="file-input" onChange={handleInput} accept="application/pdf,.md,text/markdown" className="hidden" />
      {file ? (
        <div className="flex flex-col items-center">
          ...
          <p className="text-xs text-slate-500 mt-2">{disabled ? '處理中，請先停止再更換文件' : '點擊或拖入另一份文件即可更換'}</p>
        </div>
      ) : (
        <div className="flex flex-col items-center">
