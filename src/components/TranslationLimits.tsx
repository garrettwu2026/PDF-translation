...
export default function TranslationLimits({ budgetUsd, retryLimit, estimatedUsd, onBudgetChange, onRetryLimitChange }: Props) {
  const exceedsEstimate = budgetUsd > 0 && estimatedUsd > budgetUsd;
  return (
    <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/50 p-4" data-testid="translation-limits">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="text-xs text-slate-500">費用上限（USD）
      ...
      <p className={`mt-2 text-[11px] ${exceedsEstimate ? 'text-amber-400' : 'text-slate-500'}`}>
        {exceedsEstimate ? `預估 $${estimatedUsd.toFixed(2)}，已高於目前上限。` : '到達上限會安全停止並保留進度；設為 0 表示不限額。'}
      </p>
    </div>
  );
}
